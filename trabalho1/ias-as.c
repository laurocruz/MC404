...
#define ALLOC_LINE(line) \
    line = calloc(WORD_SIZE+1, sizeof(char));
...
void inline test_eol(char * word) {
    if (token == NULL) {
...
Boolean inline make_line(BYTE * line) {
...
Boolean label_verify(char * word) {
    ...
    for (int i = 0; i < strlen(word); i++) {
        if (!((word[i] >= 0x30 && word[i] <= 0x39) ||
                    (word[i] >= 0x41 && word[i] <= 0x5A) ||
                    (word[i] >= 0x61 && word[i] <= 0x7A) || 
                    (word[i] == '_')))
            return false;
    }
...
void set_dir(Boolean rot) {
    ...
    if (!rot) {
        ...
        if (!dec_verify(token, 0, 2147483647)) {
            if (hex_verify(token)) {
                long int parameter = strtol(toke, NULL, 16);
        ...
        if (token[1] == 'x') value = strtol(token2, NULL, 16);
...
void dec_into_map(char * line, int dec) {
    int absdec = abs(dec);
...
void word_dir(BYTE ** map, unsigned short int pos[], Boolean rot) {
    ...
    if (!rot) {
        ...
        if (!dec_verify(token, 0, 2147483647) && !label_verify(token) && !hex_verify(token)) {
            // ERRO: Parametro invalido
        }
        ...
        if (dec_verify(token, 0, 2147483647)) {
            ...
            dec_into_map(map[pos[0]], abs(value)); 
        }
        pos[0]++;
    }
}
    ...
    if (!rot) {
        ...
        if (dec_verify(token, -2147483648, 2147483647)) {
            ...
            if(value == -1024) {
                value = get_value(sets, token);
                if (value == -1024) {
                    // ERRO: Rotulo ou SYM nao existem
                }
            }
            value = abs(value);
            hex = false;
...
typedef enum {LSH, RSH, JMP, JUMP+, STaddr, LD, LD-, LD|, LDmq, LDmq_mx, ST, 
    ADD, ADD|, SUB, SUB|, MUL, DIF} Instruction;

static char instructions[17][8] = 
{   {'L', 'S', 'H', '\0', 0, 0, 0, 0},
    {'R', 'S', 'H', '\0', 0, 0, 0, 0},
    {'J', 'M', 'P', '\0', 0, 0, 0, 0},
    {'J', 'U', 'M', 'P', '+', '\0', 0, 0},
...
void inline encode_instruction(BYTE * mem_word, BYTE code, int param, Boolean right) {
...
void lsh_rsh(BYTE * mem_line, int type, unsigned short int pos[], Boolean rot) {
    ...
    if (!rot) {
        ...
        if (pos[1] == 0) {
            if (make_line(mem_line)) {
                encode_instruction(mem_line, i_code[type], 0, false);
            } else {
                // ERRO: Palavra de memoria ja sendo usada
            }
            pos[1]++;
            ...
            if (!make_line(map[pos[0]])) {
                encode_instruction(mem_line, i_code[type], 0, true);
            } else {
...
void one_param_ins(BYTE * mem_line, int type, unsigned short int pos[], Boolean rot) {
    ...
    if (!rot) {
        ...
        if (!dec_verify(token, 0, 0x3FF) && !label_verify(token)) {
            if (hex_verify(token)) {
        ...
        if (pos[1] == 0) {
            if(make_line(mem_line)) {
                if (type == JMP || type == JUMP+ || type == STaddr) {
                    if (param >= 0)
                        encode_instruction(mem_line, i_code[type], param, false);
                    else encode_instruction(mem_line, i_code[type]+1, (-1)*param, false);
                } else {
                    encode_instruction(mem_line, i_code[type], abs(param), false);
                }
            } else {
                // ERRO: palavra de memoria ja sendo usada
            }
            ...
            if (!make_line(mem_line)) {
                if (type == JMP || type == JUMP+ || type == STaddr) {
                    if (param >= 0)
                        encode_instruction(mem_line, i_code[type], param, true);
                    else encode_instruction(mem_line, i_code[type]+1, (-1)*param, true);
                } else {
                    encode_instruction(mem_line, i_code[type], abs(param), true);
                }
            } else {
...
void manage_instruction(int type, BYTE ** map, unsigned short int pos[], Boolean rot) {
    ...
    if (type == LSH || type == LSH) { // Instrucoes sem parametros
        lsh_rsh(map[pos[0]], type, pos, rot);

    } else if (type >= JMP) { //Instrucoes de um parametro
        one_param_ins(map[pos[0]], type, pos, rot);
...
void inline make_address(char * add, int i) {
    add[0] = get_hex_char(i >> 16);
    add[1] = get_hex_char((i >> 8) & 0x0F);
    add[2] = get_hex_char(i & 0x0F);
}

void inline make_word(char * hex_word, BYTE * mem_line) {
...
void print_map(FILE * output, BYTE ** map) {
    ...
    for (int i = 0; i < MAP_SIZE; i++) {
        make_add(address, i);
        if (map[i] == NULL) fprintf(output, "%s 00 000 00 000\n", address);
        else {
...
int main (int argc, char **argv) {
    ...
    while (get_line(src, buffer, &buf_size) != EOF) {
        ...
        while (token != NULL || token[0] != '#') {
            if ((token[0] == '.') && !ins && !dir) { // Directive
                manage_directive(directive_type(token), map, src, pos, false);
                dir = true;
            }
    ...
    src = reopen(argv[1], "r", src);

    while(fgets(buffer, buf_size, src) != EOF) {
        token = NEW_TOKEN(buffer);

        while (token != NULL || token[0] != '#') {
            if(token[0] == '.') {
                manage_directive(directive_type(token), map, src, pos, true);
            }
